...
<head>
	...
	<script>
		...
		function pageSizeChange() {
			...
			location.href = "/guest/guestInput";
		}
...
<%-- 		<!-- 블럭페이지 시작!! -->
		...
		<div class="text-center">
			<c:if test="${pag > 1}"><a href="${ctp}/guest/guestList?pag=1&pageSize=${pageSize}" class="mr-2">First</a></c:if>
			<c:if test="${curBlock > 0}"><a href="${ctp}/guest/guestList?pag=${((curBlock-1) * blockSize)  + 1}&pageSize=${pageSize}">Before</a></c:if>		
				<c:forEach var="i" begin="${(curBlock*blockSize) + 1}" end="${(curBlock * blockSize) + blockSize}" varStatus="st">
					<c:if test="${i <= totPage && i == pag}"><strong><a href="${ctp}/guest/guestList?pag=${i}&pageSize=${pageSize}">[${i}]</a></strong></c:if>
					<c:if test="${i <= totPage && i != pag}"><a href="${ctp}/guest/guestList?pag=${i}&pageSize=${pageSize}">[${i}]</a></c:if>
				</c:forEach>
			<c:if test="${curBlock < lastBlock}"><a href="${ctp}/guest/guestList?pag=${(curBlock + 1) * blockSize + 1}&pageSize=${pageSize}">After</a></c:if>		
			<c:if test="${pag < totPage}"><a href="${ctp}/guest/guestList?pag=${totPage}&pageSize=${pageSize}" class="ml-2">Last</a></c:if>
		</div> 

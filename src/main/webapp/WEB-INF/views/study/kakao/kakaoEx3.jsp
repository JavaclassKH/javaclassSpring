...
function addressSave() {
  	...
  	$.ajax({
  		...
  		success:function(res) {
  			if(res != "0") alert("Save 5ke2");
  			else alert("Save Nope");
  		},
  		error : function() {
  			alert("전송오류!");
  		}
...
<div class="container">
  <h2>KakaoDB에 저장된 키워드검색후 MyDB에 저장하기( ${address} )</h2>
	<hr/>
	<form name="myform">
    <input type="text" name="address" id="address" placeholder="키워드를 입력하세요" autofocus required class="form-control" />
	  <div class="text-right">
		  <input type="submit" value="키워드검색" class="btn btn-outline-dark mt-3 mr-4" />
		  <input type="button" value="검색된지점을 MyDB에 저장" onclick="addressSave()" class="btn btn-outline-dark mt-3" />
	  </div><br/>
		<span id="demo"></span>

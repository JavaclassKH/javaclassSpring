...
<head>
  ...
  <script>
	  $(document).ready(function(){
			...
			//채팅 서버 주소
		  let url = "ws://192.168.50.64:9090/javaclassS/chatserver";
		  ...
		  $('#btnConnect').click(function() {
	     	...
	     	if ($('#user').val().trim() != '') {
	  	   	ws = new WebSocket(url); // 처음 접속시 유저명을 확인하고 웹소켓에 연결시켜줌
	  	   			
	  	   	// 소켓 이벤트 매핑처리(웹 소켁에 연결되면 onOpen 메소드가 수행된다)
	  	   	ws.onopen = function (evt) {
	  			...
	  			ws.onmessage = function (evt) {	 // 서버에 넘어온 값이 '2#user명: 메세지' 
		  			let index = evt.data.indexOf("#", 2);	// 메세지와 색상코드가 있는지 확인!
		  			let no = evt.data.substring(0, 1); // 접속자 상태 판별 ( 1(첫접속),2(기존접속),3(종료) )
...
<div class="container">
	...
	<div style="height:480px;border:1px solid #fff;border-radius:4px;margin:2px 0;background-color:#F5ECCE">
		<div id="list"></div>
	</div>
	<div>
		<div>
		  <textarea name="msg" id="msg" rows="4" style="resize: none; margin-top: 70px;" placeholder="대화 내용을 입력하세요." class="form-control mb-2" disabled></textarea>
		</div>
	</div>
